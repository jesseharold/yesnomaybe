<template>
  <section id="items" :class="compareMode ? 'compare-mode' : ''">
    <h3 v-if="compareMode" class="hide-print">
      Show only items that have this level of interest or higher:
    </h3>
    <Slider
      v-if="compareMode"
      class="hide-print"
    ...
    <div class="row columns column-labels">
      <div class="column activity">
        <button class="toggle-button hide-print" @click="openAllCategories">
          Open all
        </button>
        <button class="toggle-button hide-print" @click="closeAllCategories">
          Close all
        </button>
      </div>
      <div
    ...
    >
      <!-- categories -->
      <h2 class="category-title" @click="toggleCategory(getName(category))">
        {{ getName(category) }}
        <button
          class="category-toggle hide-print"
          title="Expand/Close this Category"
          :name="getVis(category) ? 'close' : 'open'"
        >
          {{ getVis(category) ? "^" : "V" }}
        </button>
      </h2>
      ...
      <div
        class="row columns"
        :class="filteredOut(item.id) ? 'hidden' : ''"
      ...
      <div class="add-custom hide-print row columns">
        ...
        />
        <button
          class="new-item column"
          @click="addItem(newCustomName, getName(category))"
        >
          Add item to {{ getName(category) }}
        </button>
      </div>
    </div>
    <!-- add new category -->
    <div class="add-custom add-custom-category hide-print row columns">
      ...
      />
      <button
        class="new-item new-category column"
  ...
  <div class="control-panel hide-print">
    ...
    <textarea
      ...
      cols="100"
      rows="4"
      placeholder="open your saved ynm.txt file in a *plain text editor* and paste it here"
    />
    <button
      class="load-button"
      :class="canLoad ? 'enabled' : 'disabled'"
      @click="loadJson"
    >
...
export default {
    ...
    categorizedItems() {
      ...
      for (const cat in categorized) {
        if (Object.hasOwnProperty.call(categorized, cat)) {
          categorizedAndSorted[cat] = util.alphabetize(
            categorized[cat],
            "label"
          );
        }
  ...
  methods: {
    setInitialCategoryVis() {
      ...
      return sortedCategories.reduce(function (o, val) {
        o[val] = false;
        return o;
      }, {});
    },
    ...
    addItem(name, categoryName) {
      ...
      if (
        ...
        this.categoryOpen[categoryName] !== false
      ) {
    ...
    compareTwo(compareToItems) {
      ...
      // rename columns
      this.columns.filter((col) => col.id === "giving")[0].label =
        p1Name + " receiving";
      this.columns.filter((col) => col.id === "receiving")[0].label =
        p2Name + " receiving";
    ...
    filteredOut(itemId) {
      ...
      return (
        (row.giving || 0) < this.compareThreshold &&
